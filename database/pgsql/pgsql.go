...
// GormPGSqlByDSN 根据DSN初始化数据库
func GormPGSqlByDSN(dsn string) (*gorm.DB, error) {
	var gormCfg gorm.Config
	db, err := gorm.Open(postgres.New(postgres.Config{
		DSN: dsn,
	}), &gormCfg)
